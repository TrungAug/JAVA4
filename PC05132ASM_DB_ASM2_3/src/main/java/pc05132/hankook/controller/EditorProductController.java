	...
	protected void doDeleteProduct(HttpServletRequest req, HttpServletResponse resp)
		...
		if (checkProduct != null) {
			...
			try {
				...
				List<Share> listS = HankookUntils.excuteNamedQuery("paramId", idProd, "Share.findByIdProd", Share.class);
				for (Share share : listS) {
					ShareDAO.getInstance().remove(share.getIdShare()+"");
				}
				
				
				List<Like> listL = HankookUntils.excuteNamedQuery("paramId", idProd, "Like.findByIdProd", Like.class);
				for (Like like : listL) {
					LikeDAO.getInstance().remove(like.getIdLike()+"");
				}	
